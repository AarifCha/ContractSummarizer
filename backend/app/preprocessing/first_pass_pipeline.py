...
from app.preprocessing.chunk_vector_index import index_chunks_into_chromadb
from app.preprocessing.first_pass_metadata_extraction import (
...
def _run_first_pass_local(
    ...
    *,
    ...
    """Docling → gc → unpack/reindex → gc → Chroma indexing; propagate OSError / MemoryError."""
    ...
    try:
        ...
        gc.collect()
        unpack_dense_list_chunks(output_dir, stem)
        gc.collect()
        return index_chunks_into_chromadb(
            output_dir,
            stem,
...
def run_first_pass_for_pdf(*, pdf_id: int, user_id: int, source_pdf_path: Path) -> None:
    ...
    total_batches = estimate_docling_batch_count(source_pdf_path.resolve())
    ...
    try:
        ...
        def on_batch_progress(completed: int, total: int) -> None:
            # Keep DB progress aligned with Docling's internal batch loop.
            set_processing_state(
                pdf_id,
                total_chunks=total,
        ...
        def on_embedding_progress(completed: int, total: int) -> None:
            nonlocal embedding_stage_started
