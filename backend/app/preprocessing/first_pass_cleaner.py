...
def _normalize_line_text(text: str) -> list[str]:
    t = (text or "").strip("\n")
    if t == "":
        return [""]
    return t.split("\n")
...
def _append_split_stats_to_audit(output_dir: Path, split_stats: dict[str, Any]) -> None:
    ...
    if p.exists():
        try:
            ...
            base = {}
    base["line_split"] = split_stats
    base.setdefault("generated_at", datetime.now(timezone.utc).isoformat())
    p.write_text(json.dumps(base, ensure_ascii=False, indent=2), encoding="utf-8")


def _expand_noncombined_chunk(data: dict[str, Any]) -> tuple[list[dict[str, Any]] | None, str | None]:
    lines = _normalize_line_text(str(data.get("text") or ""))
    bboxes = data.get("bboxes") or []
    if not isinstance(bboxes, list):
        return None, "line_bbox_mismatch"
    if len(lines) != len(bboxes):
        return None, "line_bbox_mismatch"

    def _base_payload() -> dict[str, Any]:
        out: dict[str, Any] = {
            "headings": list(data.get("headings") or []),
            "page_numbers": list(data.get("page_numbers") or []),
        }
        if "filterCombined" in data and not bool(data.get("filterCombined")):
            out["filterCombined"] = False
        return out

    def _emit(text_out: str, bbox_items: list[Any]) -> dict[str, Any]:
        p = _base_payload()
        p["text"] = text_out
        p["bboxes"] = bbox_items
        return p
    ...
    while i < n:
        ...
        if wc >= 8:
            out_chunks.append(_emit(line, [bbox_i]))
            i += 1
            continue
        ...
        if i == n - 1:
            if out_chunks:
                ...
                prev_bboxes = prev.get("bboxes") if isinstance(prev.get("bboxes"), list) else []
                prev_bboxes.append(bbox_i)
                prev["bboxes"] = prev_bboxes
            else:
                out_chunks.append(_emit(line, [bbox_i]))
            i += 1
            continue
        ...
        wc2 = _word_count(next_line)
        if wc + wc2 >= 8:
            out_chunks.append(_emit(f"{line}\n{next_line}", [bbox_i, bboxes[i + 1]]))
            i += 2
            continue
...
def combine_suspect_pages(output_dir: Path, stem_hint: str | None = None) -> dict[str, Any]:
    ...
    for page_num in sorted(pages.keys()):
        ...
        word_counts = [_word_count(str(data.get("text") or "")) for _, data in items]
...
def split_noncombined_chunks_by_line(output_dir: Path, stem: str) -> dict[str, Any]:
    """
    Split non-combined chunks by text line with bbox alignment rules:
    - process only chunks where filterCombined != True
    - skip chunk unchanged if line count != bbox count
    """
    output_dir = output_dir.resolve()
    files = _chunk_json_paths_for_stem(output_dir, stem)
    if not files:
        stats = {
            "processed_chunks": 0,
            "skipped_filter_combined": 0,
            "skipped_line_bbox_mismatch": 0,
            "input_chunks": 0,
            "output_chunks": 0,
        }
        _append_split_stats_to_audit(output_dir, stats)
        return stats
    ...
    for fp in files:
        ...
        if not data:
            continue
        if bool(data.get("filterCombined")):
            skipped_filter_combined += 1
            ordered_payloads.append(data)
            continue

        input_chunks += 1
        expanded, reason = _expand_noncombined_chunk(data)
        if expanded is None:
            if reason == "line_bbox_mismatch":
                skipped_line_bbox_mismatch += 1
            ordered_payloads.append(data)
            continue
        if not expanded:
            ordered_payloads.append(data)
        ...
        processed_chunks += 1
        ordered_payloads.extend(expanded)
        output_chunks += len(expanded)

    # Rewrite stem chunk files in strict sequence order so split parts stay adjacent
    # to the source chunk position (e.g. 11a, 11b, then old 12).
    tmp_paths: list[Path] = []
    for i, fp in enumerate(files):
        tmp = output_dir / f"_split_tmp_{i:05d}.json"
        fp.rename(tmp)
        tmp_paths.append(tmp)

    for i, payload in enumerate(ordered_payloads, start=1):
        payload["chunk_index"] = i
        out_path = output_dir / f"{stem}_chunk_{i:03d}.json"
    ...
    stats = {
        ...
        "skipped_filter_combined": skipped_filter_combined,
        "skipped_line_bbox_mismatch": skipped_line_bbox_mismatch,
        "input_chunks": input_chunks,
        "output_chunks": output_chunks,
    }
    _append_split_stats_to_audit(output_dir, stats)
    logger.info("first_pass_cleaner: line split stats %s", stats)
    return stats
